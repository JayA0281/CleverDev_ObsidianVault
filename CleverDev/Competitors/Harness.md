https://www.harness.io/
```table-of-contents
```
## What does it do?
- its an all-in-one AI powered DevOps platform
- focused on:
	- CI/CD
	- Automation
	- Developer productivity
	- cloud cost optimization
*they are basically trying to own everything after code is written*

### About the platform: Harness is a modular platform
#### core modules:
- CI
- CD
- Feature FLags
- Cloud Cost Management
- Security testing(DevSecOps)
- Engineering Insights
#### AI for DevOps & Automation
- CD & GitOps
- CI
- Internal Developer Portal
- Infrastructure as code management
- Database Devops
- Artifact Regsitry
#### AI for Testing & Resilience
- Feature Management & Experimentation
- Resilience Testing
- AI Test Automation
- AI SRE
#### AI for Security & Compliance
- Application Security Testing
- Web Application & API Protection
- AI Security
#### AI for Cost & Optimization
- Cloud Cost Management
- Software Engineering Insights

### Integrate first, then remove other tools
#### Integrations:
- GitHub, GitLab, Bitbucket
- AWS, Azure, GCP
- Kubernetes
- Artifact registries
- Monitoring tools

At first, Harness sits in the middle integrating with your tools, soon removing the need of those tools at all

they start with 1 or 2 modules(usually CI/CD), gradually expanding

so it is a seperate tool/platform where teams work, but it also integrates with your existing tools

